...
Pstart = ceil(0.25*fs)-1; %ceil(0.25*fs)-1;
Tstop = floor(0.45*fs);%floor(0.40*fs); %floor(0.45*fs);
ecg_template_samples = Pstart + Tstop + 1;
ecg_template_seconds = ecg_template_samples * 1 / fs;
ecg_buff = zeros(round(ecg_template_seconds*fs),NB_CYCLES); % ecg stack buffer
% == template ecg (TECG)
...
% eliminate ecg residuals
cos_len = 0.1*length(TECG);
x = 0:pi/cos_len:pi;
cos_part=(cos(x)+1)/2;
raised_cos=ones(1,length(TECG));
raised_cos(length(raised_cos)-length(cos_part)+1:length(raised_cos)) = cos_part;
TECG=raised_cos'.*TECG;
    ...
    if peaks(qq)>Pstart && length(ecg)-peaks(qq)>Tstop
        switch method
            ...
            case 'TS-SUZANNA'
                ...
                M  = zeros (round(ecg_template_seconds*fs),3);
...
if debug
    ...
    figure('name',[method ' extraction1']);
    ...
    rIndex = qrs_adjust(ecg,peaks,fs,abs(max(ecg))> abs(min(ecg)),0.05,0); %DEBUG_NOAM
    plot(tm(rIndex),ecg(rIndex),'+k','LineWidth',2); %DEBUG_NOAM
    legend('Original signal','estimated signal','orig_qrs','corrected_qrs');
    ...
    sgtitle(['NB CYCLES = ' num2str(NB_CYCLES)]);
    xlabel('Time [sec]'); ylabel('Amplitude [NU]')
    set(gca,'FontSize',FONT_SIZE);
    set(findall(gcf,'type','text'),'fontSize',FONT_SIZE);
    figure('name','Template');
    tm = 1/fs:1/fs:length(TECG)/fs;
    plot(tm,TECG,'LineWidth',LINE_WIDTH)
    hold on
    plot(tm,ecg_temp(peaks(5)-Pstart:peaks(5)+Tstop), '--r','LineWidth',LINE_WIDTH)
    hold on
    xlabel('Time [sec]'); ylabel('Amplitude [NU]')
    legend('template', 'ECG cycle');
    set(gca,'FontSize',FONT_SIZE);
end
end
